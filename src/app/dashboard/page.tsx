...
export default function DashboardPage() {
  ...
  function getCurrentCycle(progresses: Progress[]): number {
    ...
    const daysInMaxCycle = completed.filter(
      (p) => p.cycle === maxCycle
    ).length;
  ...
  const athleteList = athletes.filter((u) =>
    u.code?.startsWith("ATL")
  );
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div
              style={{
                ...
                borderRadius: 6,
                background:
                  "linear-gradient(135deg,#2B6CB0,#4A90D9)",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                fontSize: 14,
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "48px" }}>
        ...
        >
          {[
            ...
            {
              label: "Progresso médio (ciclo atual)",
              value: `${avgProgress}%`,
              icon: "📊",
            },
          ].map((card) => (
                ...
                <tbody>
                  {athleteList.map((user, i) => {
                    ...
                    const currentCycleDays = user.progresses
                      .filter(
                        (p) =>
                          p.completed && p.cycle === currentCycle
                      )
                      .map((p) => p.day);

                    const pct = Math.round(
                      (currentCycleDays.length / 7) * 100
                    );

                    const totalCycles = Object.keys(cycles).length;

                    const completedCycles = Object.values(cycles).filter(
                      (days) => days.length >= 7
                    ).length;
